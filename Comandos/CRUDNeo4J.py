...
class ComandosNeo4J:
    ...
    def _read_nodes_Ejercicio1(tx, empresa):
        query = (
        "MATCH (n:persona)-[t:trabaja_en]->(:empresa {name: $empresa}) "
        ...
        for record in result:
            persona = record["n"]["name"]  # O ajusta si el atributo 'name' no existe
            roles = record["roles"]  # Lista de roles para esa persona
            resultados[persona] = roles
    ...
    def _read_nodes_Ejercicio2(tx):
        query = (
        "MATCH (n:persona)-[t:trabaja_en]->(e:empresa), (m:persona)-[k:trabaja_en]->(z:empresa) "
    ...
    def _read_nodes_Ejercicio3(tx):
        query = (
        "MATCH (n:persona)-[:trabaja_en]->(e:empresa)<-[:trabaja_en]-(m:persona) "
        "WHERE n <> m "
        ...
        personas = []
        for record in result:
            personas.append(record["empresa"])
